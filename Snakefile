...
def find_raw_fastq(wildcards):
    """Locate the one raw FASTQ file for a sample (name must start with sampleid_)."""
    matches = glob.glob(f"{config['raw_fastq_dir']}/{wildcards.sample}_*.fastq*")
    if len(matches) != 1:
        raise ValueError(
...
rule trim:
    ...
    shell:
        ...
        "ref={config[truseq_ref]},{config[polya_ref]} "
...
rule fastqc:
    ...
    shell:
        "fastqc -t {config[threads]} --nogroup -o {params.outdir} {input} > {log} 2>&1"
...
rule promote_trimmed:
    ...
    output:
        f"{config['results_dir']}/trim/{{sample}}_trimmed.fastq"
    shell:
        "cp {input} {output}"
...
rule align_count:
    ...
    shell:
        "STAR --runThreadN {config[threads]} --genomeDir {config[genome_dir]} "
...
rule collect_counts:
    ...
    shell:
        "python3 pipeline/format_counts_table.py {params.star_dir} -o {params.out_dir}"
